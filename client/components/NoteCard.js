...
const NoteCard = ({ note }) => {
  ...
  return (
    <div className="border border-gray-300 shadow-lg p-4 rounded-lg mx-auto">
      <h3 className="text-2xl md:text-xl font-semibold mb-2 text-center md:text-left">{note.question}</h3>
      {note.content.length > 0 ? (
        note.content.map((item, index) => {
          if (item.type === "answer") {
            return <p key={index} className="mb-2 text-2xl md:text-lg">{item.value}</p>;
